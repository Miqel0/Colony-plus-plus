...
* **Energia:** Bez niej żaden z budynków produkcyjnych nie będzie działać (aczkolwiek gra się nie kończy).
...
Gra korzysta z zewnętrznych plikach tekstowych, w których są zapisywane informaacje podczas zapisywania gry, albo są trzymane parametry samych budynków (w celu ułatwienia zmiany parametrów budynków):

* **`gamedata.txt`**: "Baza danych" używanych budynków w grze. Zawiera statystyki wszystkich dostępnych budynków (koszty, produkcja, wymagany poziom, opis). Dzięki temu balansowanie gry nie wymaga rwprowadzania licznych zmian w kodzie.
...
2.  Pobierz najnowsze archiwum `.zip` (np. `v0.0.1-alpha`).
3.  **Wypakuj całe archiwum** do dowolnego folderu.
4.  **Ważne:** Upewnij się, że plik `gamedata.txt` znajduje się w folderze `src`.
...
| **`rename`** | *-brak-* | Pozwala zmienić nazwę kolonii (uwzgędniając użycie `_`, co następnie jest zastępowane spacją np. `rename New_Vegas` -> `New Vegas`. |
